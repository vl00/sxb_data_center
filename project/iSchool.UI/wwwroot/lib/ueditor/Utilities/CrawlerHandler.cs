...
public class CrawlerHandler : Handler
{
    ...
    public override ActionResult Process
    {
        get
        {
            ...
            Guid.TryParse(Request.QueryString["articleID"], out Guid articleID);
            Crawlers = Sources.Select(x => new Crawler(x, Server).Fetch(articleID)).ToArray();
            return WriteJson(new
            {
                state = "SUCCESS",
...
public class Crawler
{
    ...
    public Crawler Fetch(Guid articleID)
    {
        ...
        using (var response = request.GetResponse() as HttpWebResponse)
        {
            ...
            ServerUrl = PathFormatter.Format(Path.GetFileName(this.CatchUrl), Config.GetString("catcherPathFormat"), out Guid fileID, articleID);
            FileID = fileID;
            ...
            try
            {
                ...
                {
                    iSchool.Library.isCommon.TransportFile(bytes, ServerUrl, out string ext, "article", null);
                    ServerUrl += ext;
                }
                else
                {
                    iSchool.Library.isCommon.TransportFile(bytes, ServerUrl, "", "article", null);
                }
