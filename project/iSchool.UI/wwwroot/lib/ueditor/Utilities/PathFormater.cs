...
{
    public static string Format(string originFileName, string pathFormat, out Guid fileID, Guid articleID = default(Guid))
    {
        ...
        pathFormat = pathFormat.Replace("{guid}", fileID.ToString());
        pathFormat = pathFormat.Replace("{articleID}", articleID.ToString());
