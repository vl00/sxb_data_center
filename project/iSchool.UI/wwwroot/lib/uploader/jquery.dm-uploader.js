...
//格式校验
(function (window) {
    if (!window.BenExt) window.BenExt = {};
    var PostfixUtil = function () {
        this.imageExt = ["png", "jpeg", "jpg","bmp","gif"];
        this.postfixToClass = [];
        ...
        this.IsImage = function (userFileName) {
            //获取最后一个.的位置
            var index = userFileName.lastIndexOf(".");
            if (-1 == index)
                return false;
            //获取后缀
            var ext = userFileName.substr(index + 1).toLowerCase();
            return this.imageExt.indexOf(ext) == -1 ? false : true;
        }
    };
    window.BenExt.postfixUtil= new PostfixUtil();
})(window);
  ...
  DmUploader.prototype.addFiles = function(files)
  {
    ...
    for (var i= 0; i < files.length; i++)
    {
      ...
      if (!file){
        ...
        var can_continue = this.settings.onNewFile.call(this.element, file.id, file.data, this.settings);
