...
(function (jQuery) {
	...
	jQuery.fn.ajaxChosen = function (ajaxOptions, options, chosenOptions) {
		...
		chosen = select.next();
		input = jQuery('input', chosen);
		inputBG = input.get(0) ? input.get(0).style.background : '';
		//copy out success callback
		if ('success' in ajaxOptions && jQuery.isFunction(ajaxOptions.success)) {
		...
		ajaxOptions.success = function (data, textStatus, jqXHR) {
			...
			if (requestQueueLength > 1) {
				jQuery.each(requestQueue, function (idx, elem) {
					if (data.q === elem) {
			...
			inputEmptiedValue = jQuery.trim(input.val());//if we have a minLength > 1 then we have to preserve the value (TODO::watch out for potential XSS/other breakages)
			inputEmptied = inputEmptiedValue.length < minLength;

			//if additional processing needs to occur on the returned json
			if ('processItems' in options && jQuery.isFunction(options.processItems)) {
			...
			selected = jQuery('option:selected', select).not(':empty').clone().attr('selected', true);
			//saving values for deduplication
			if (!jQuery.isArray(select.val())) {
			...
			jQuery('<option value=""/>').appendTo(select);
			...
			if (!inputEmptied) {
				if (jQuery.isArray(items)) {
					...
					jQuery.each(items, function (i, opt) {
						if (typeof valuesHash[opt.id] === 'undefined') {
							jQuery('<option value="' + opt.id + '">' + opt.text + '</option>').appendTo(select);
						}
					});
				} else {
					//hash of kv pairs {'id':'text'...}
					jQuery.each(items, function (value, text) {
    ...
    jQuery('.chosen-search > input, .chosen-choices .search-field input', chosen).
    ...
    function processValue(e) {
        ...
        (e.which && (
			...
			if ('useAjax' in options && jQuery.isFunction(options.useAjax)) {
				if (!options.useAjax(e)) { return false; }
			}
			//hide no results
			jQuery('.no-results', chosen).hide();
			//backout if nothing is in input box
			if (jQuery.trim(q).length < minLength) {
			...
			//add query to data
			if (jQuery.isArray(ajaxOptions.data)) {
				//array
				...
				jQuery.extend(ajaxOptions.data, { data: {q: q} });
			}
			//dynamically generate url
			if ('generateUrl' in options && jQuery.isFunction(options.generateUrl)) {
