...
{
    public class SearchSachSchoolQueryHandler : IRequestHandler<SearchSachSchoolQuery, List<KeyValueDto<Guid>>>
    {
        private IMediator mediator;
        private UnitOfWork UnitOfWork { get; set; }

        public SearchSachSchoolQueryHandler(IMediator mediator, IUnitOfWork unitOfWork)
        {
        ...
        public async Task<List<KeyValueDto<Guid>>> Handle(SearchSachSchoolQuery request, CancellationToken cancellationToken)
        {            
            if (request.IsCollage)
            {
            ...
            else
            {
                ...
                var sql = "select (s.name+'_'+e.name)as [key],e.id as [value] from school s join schoolextension e on s.id=e.sid " +
                    $"where e.IsValid=1 and s.id in @sids {"and e.grade=@Grade".If(request.Grade != 0)}";

                var ls = await UnitOfWork.DbConnection.QueryAsync<KeyValueDto<Guid>>(sql, new { sids, request.Grade });
