...
{
    public class KVQueryHandler : IRequestHandler<KVQuery, KeyValue[]>
    {
        const string prev_key = @"kvdata";
        ...
        public KVQueryHandler(IUnitOfWork unitOfWork, CacheManager cacheManager, ILog log, IMemoryCache memoryCache)
        {
        ...
        public async Task<KeyValue[]> Handle(KVQuery req, CancellationToken cancellationToken)
        {
            KeyValue[] ress = null;
            string json = null;
            string[] ss = null;
            ...
            ress = memoryCache.Get<KeyValue[]>(CacheManager.StrToMD5(_key));
            ...
            try
            {
                ...
                //{
                //    return ress = json.ToObject<KeyValue[]>();
                //}
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                var sql = @"select * from [dbo].[KeyValue] where [type]=@Type and parentid=@ParentId and IsValid=1";

                ress = (await unitOfWork.DbConnection.QueryAsync<KeyValue>(sql, req).ConfigureAwait(false)).ToArray();
